...
import { ApprovalType } from '@anasuapp/controller-utils';
...
const store = configureStore({
  anasuapp:{ ...mockState.anasuapp, pendingApprovals:{ 1 : pendingNftApprovals[1] }}
});
const duplicateAddressStore = configureStore({
 anasuapp:{ ...mockState.anasuapp, pendingApprovals :pendingNftApprovals}
});
...
WithMultipleSuggestedNFTs.decorators=[
 story => <Provider store={duplicateAddressStore}>{story()}</Provider>
];
